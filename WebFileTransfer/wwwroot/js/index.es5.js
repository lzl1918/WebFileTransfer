...
(function (window) {
    ...
    var size2str = function size2str(length) {
        if (length > 1073741824 * 0.5) return (length / 1073741824.0).toFixed(2) + "GB";
        if (length > 1048576 * 0.5) return (length / 1048576.0).toFixed(2) + "MB";
        if (length > 1024 * 0.5) return (length / 1024.0).toFixed(2) + "KB";
        return length + "B";
    };

    var directory_click = function directory_click(e) {
    ...
    var file_click = function file_click(e) {
        var path = getpath(null);
        var item = e.currentTarget;
        var itemname = e.currentTarget.children[1].innerText;
        path = path + "\\" + itemname;
    ...
    var path_mouse_enter = function path_mouse_enter(e) {
        ...
        if (i > 0) $(chds[i - 1]).addClass("hovered-before");
    };
    var path_mouse_out = function path_mouse_out(e) {
        var chds = path_container.children;
        var item = e.currentTarget;
        var i = 0;
        for (i = 0; i < chds.length; i++) {
        ...
        if (i > 0) $(chds[i - 1]).removeClass("hovered-before");
    };
    var path_click = function path_click(e) {
        var item = e.currentTarget;
        var path = getpath(item);
        query_files(path);
    };

    var produce_path_div = function produce_path_div(name) {
    ...
    var produce_directory_div = function produce_directory_div(directory) {
        var div = document.createElement("div");
    ...
    var produce_file_div = function produce_file_div(file) {
        var div = document.createElement("div");
        var img = document.createElement("img");
        var span_name = document.createElement("span");
        var span_size = document.createElement("span");
    ...
    var query_files = function query_files(path) {
        var param = { path: path };
        $.ajax({
    ...
    var down_files = function down_files(path) {
        var param = { path: path };
    ...
    var getpath = function getpath(atbefore) {
        var chds = path_container.children;
        var path = "";
        var i = 0;
        var tpath;
        if (atbefore == chds[0]) return "";

        for (i = 1; i < chds.length; i++) {
            tpath = chds[i].innerText;
            if (tpath[tpath.length - 1] == '\n' || tpath[tpath.length - 1] == '\r') tpath = tpath.substr(0, tpath.length - 1);
            path += tpath + "\\";
            if (chds[i] == atbefore) break;
        }
        if (path.length > 0) path = path.substr(0, path.length - 1);
        return path;
    };

    var makefilter = function makefilter() {
    ...
    var filter_files = function filter_files(filter) {
        list_dirs.innerHTML = "";
        ...
        if (data_directories) {
            for (var i = 0; i < data_directories.length; i++) {
                if (data_directories[i].name.match(exp)) items_dirs.push(data_directories[i]);
            }
        }
        if (data_files) {
            for (var i = 0; i < data_files.length; i++) {
                if (data_files[i].name.match(exp)) items_files.push(data_files[i]);
            }
        }
