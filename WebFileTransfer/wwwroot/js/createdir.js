(function (window) {
    ...
    var set_attr = function (obj, prop, val) {
        obj.setAttribute("data-" + prop, val);
    };
    var remove_attr = function (obj, prop) {
        if (obj.hasAttribute("data-" + prop))
            obj.removeAttribute("data-" + prop);
    };

    var dirs = undefined;
    var illegalmatch = /[\\\/\:\*\?\"\<\>\|]/;
    var creating = false;
    var flyout_showed = function (sender, args) {
    ...
    var flyout_hiding = function (sender, args) {
        var flyout = args.flyout;
        if (creating) {
